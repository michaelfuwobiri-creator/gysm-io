...
// as its own app -- distinct name/icon -- instead of a GYSM bookmark.
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const description = tagline || `${title} -- an app built with GYSM. Describe an app, get a real one.`;
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || "https://www.gysm.io"}/publish/${params.id}`;

  return {
    title: `${title} — built with GYSM`,
    description,
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            operatingSystem: "Web",
            description: `${project.prompt} -- built with GYSM.`,
          }),
        }}
      />
      <iframe
      ...
      <div className="shrink-0 flex items-center justify-between gap-3 px-4 py-2 bg-white border-t border-black/10 text-black/40 text-[11px]">
        <div
          ...
          title="This app was generated by artificial intelligence from a text prompt, using GYSM."
        >
        ...
        {project.check_status === "pass" && (
          <span
            title="Passed GYSM's automated pre-publish check -- no broken internal links, no unclosed tags, no leftover placeholder text."
